...
type octetStreamRT struct {
	RoundTripper http.RoundTripper
}

func (c *octetStreamRT) RoundTrip(req *http.Request) (*http.Response, error) {
...
func (up *Updater) UpdateTo(rel *Release, cmdPath string) error {
	hc := *up.httpClient
	hc.Transport = &octetStreamRT{hc.Transport}
	src, redirectURL, err := up.api.Repositories.DownloadReleaseAsset(up.apiCtx, rel.RepoOwner, rel.RepoName, rel.AssetID, &hc)
	if err != nil {
	...
	validationSrc, validationRedirectURL, err := up.api.Repositories.DownloadReleaseAsset(up.apiCtx, rel.RepoOwner, rel.RepoName, rel.ValidationAssetID, &hc)
	if err != nil {
